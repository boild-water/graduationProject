				...
				<div class="input-search">
					<div>
						...
						<dl class="layui-hide-sm layui-show-md-inline-block"> <dt>热搜：</dt>
							<dd><a href="<%=basePath%>goods/category/0?search=充电宝">充电宝</a></dd>
							<dd><a href="<%=basePath%>goods/category/0?search=iPhone 11Pro">iPhone 11Pro</a></dd>
							<dd><a href="<%=basePath%>goods/category/0?search=高等数学">高等数学</a></dd>
							<dd><a href="<%=basePath%>goods/category/0?search=机械键盘">机械键盘</a></dd>
							<dd><a href="<%=basePath%>goods/category/0?search=java 编程思想">java 编程思想</a></dd>
							<dd><a style="color: #FF5722;">全部</a></dd>
						</dl>
					</div>
		...
		<div class="class6">
			<div class="class4">
				...
				<form class="layui-form" action="">
					<legend class="class7">手机号登录</legend>
					<div class="layui-form-item">
						...
						<div class="layui-inline" style="margin-bottom: 20px;width: 100%;position: relative;">
							<div class="layui-input-inline" style="width: 100%;position: relative;">
								<i class="layui-icon layui-icon-password" id="passwordTips" style="position: absolute;top: 8px;left: 10px;color: #d3d3d3;font-size: 20px;"></i>
					...
					<div class="layui-form-item">
						<div class="layui-input-block" style="margin-left: 0;">
							<button type="submit" class="layui-btn" lay-submit="" lay-filter="loginBtn" style="width: 100%;background: #cfb2f6;font-size: 18px;">登录</button>
						</div>
					</div>
					<div class="layui-form-item">
						还没有账号？<a href="<%=basePath%>user/register">点击注册</a>
					</div>
				</form>
			</div>
		</div>
				...
				success: function (result) {
					if (result.flag){//表示操作成功
						layer.msg(result.message, {
							...
							if($.trim(prevLink).length < 0){
								//如果是地址栏直接输入进入登录页面，就跳转到首页
								location.href = '<%=basePath%>goods/homeGoods';
								 ...
								 */
								//是否来自其它站点
								if(prevLink.indexOf('localhost:8080') == -1){
									location.href = '<%=basePath%>goods/homeGoods';
								}
								//是否来自注册页面
								if(prevLink.indexOf('user/register') != -1){
									location.href = '<%=basePath%>goods/homeGoods';
								} else {
