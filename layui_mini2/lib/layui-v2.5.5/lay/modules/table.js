	...
	j.prototype.config = {
			...
			i.layBody.on("change", "." + W, function() {
				...
				if (!i.data("off") && a) {
					...
					// var n = t('<input class="layui-input ' + W + '">');//源代码
					
					// update by boildwater 2020-05-08; 修改数据表格单元格编辑只有text，添加textarea，适应于长文本编辑。
					var n = i.data('edit') == 'textarea' ? t('<textarea class="layui-textarea '+W+'"></textarea>') : t('<input class="layui-input '+W+'">');
					
					// 此处想尝试添加laydate组件，也作为单元格修改类型，未果。
					// var n = i.data('edit') == 'textarea' ? t('<textarea class="layui-textarea '+W+'"></textarea>') 
						// : (i.data('edit') == 'date' ? t('<input class="mydate layui-input  '+W+'"></input>') 
						// 	: t('<input class="layui-input '+W+'">'));
